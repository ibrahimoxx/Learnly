...
const RARITY_COLORS: Record<string, string> = {
  common: "text-[--color-text-muted] border-[--color-border] bg-[--color-surface-raised]",
  uncommon: "text-[--color-primary] border-[--brand-200] bg-[--color-primary-subtle]",
  rare: "text-[--brand-800] border-[--brand-300] bg-[--color-primary-subtle]",
  epic: "text-[--color-warning] border-[--color-warning]/30 bg-[--color-warning]/10",
  legendary: "text-[--color-error] border-[--color-error]/30 bg-[--color-error]/10",
};
  ...
  return (
    <div className="premium-shell min-h-screen px-4 py-10 sm:px-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div className="hover-lift premium-card rounded-[--radius-md] p-4 text-center">
        ...
        <div className="hover-lift premium-card rounded-[--radius-md] p-4 text-center">
          <Flame className="mx-auto h-6 w-6 text-[--color-warning] mb-1" />
        ...
        <div className="hover-lift premium-card rounded-[--radius-md] p-4 text-center">
          <Calendar className="mx-auto h-6 w-6 text-[--color-accent-mint] mb-1" />
      ...
      {/* XP progress bar */}
      <div className="premium-card mt-6 rounded-[--radius-md] p-4">
      ...
      {/* Badge grid */}
      <h2 className="mt-8 text-2xl font-black tracking-tight text-[--color-text-primary]">
      ...
      <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
        {ALL_BADGES.map((badge) => {
          ...
          const rarityClass = earned ? (RARITY_COLORS[badge.rarity] ?? RARITY_COLORS.common) : "text-[--color-text-muted] border-[--color-border] bg-[--color-surface]";

          return (
            <div
              key={badge.code}
              className={`rounded-[--radius-md] border p-4 text-center shadow-[var(--shadow-xs)] transition-all ${rarityClass} ${earned ? "" : "opacity-60 grayscale"}`}
            >
